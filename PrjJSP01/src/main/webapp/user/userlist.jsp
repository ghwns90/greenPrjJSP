<%@page import="java.sql.ResultSet"%>
<%@page import="java.sql.PreparedStatement"%>
<%@page import="java.sql.DriverManager"%>
<%@page import="java.sql.Connection"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
    
<% 
	// 넘어오는 값 받기
	// http://localhost:8080/PrjJSP01/userlist.jsp?name=all
	String name = request.getParameter("name");
	System.out.println(name);
	
	if(name == null) name = "all";
	
	// 연결 문자열
	String driver = "oracle.jdbc.OracleDriver";
	String dburl  = "jdbc:oracle:thin:@localhost:1521:xe";
	String dbuid  = "test";
	String dbpwd  = "1234";
	
	
	// db 조회
	Class.forName(driver);
	Connection conn		    = DriverManager.getConnection(dburl, dbuid, dbpwd);
	
	String tag = "";
	// name 값에 담겨있는 문자열에 따라서 sql문이 달라진다
	if(name.equals("all")){
		tag += "<table id='t2'>";
		// html 페이지에서 "전체조회"를 누르면 name 에 all 이 담겨져 온다
		String sql = "";
		sql       += "SELECT id, name, userid, passwd, email, to_char(regdate, 'yyyy-MM-dd, DY') as regdate FROM TUSER";
		sql       += " ORDER BY ID ASC";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		ResultSet rs            = pstmt.executeQuery();
		
		
		while(rs.next()){
			
			tag += "<tr>";
			tag += "<td>" + rs.getInt("id") + "</td>";
			tag += "<td>" + rs.getString("name") + "</td>";
			tag += "<td>" + rs.getString("userid") + "</td>";
			tag += "<td>" + rs.getString("passwd") + "</td>";
			tag += "<td>" + rs.getString("email") + "</td>";
			tag += "<td>" + rs.getString("regdate") + "</td>";
			tag += "<td><a href='deluser.jsp?id=" + rs.getInt("id") + "'>❌</a></td>";
			tag += "<tr>";			
		
		}
		tag += "</table>";
		
		rs.close();
		pstmt.close();
		conn.close();
		
	}else {
		
		tag += "<table id = 't2'>";
		// html 페이지에서 이름을 입력하면 name 에 입력한 값이 담겨져 온다		
		
		String sql = "";
		sql       += "SELECT id, name, userid, passwd, email, to_char(regdate, 'yyyy-MM-dd, DY') regdate FROM TUSER";
		sql       += " WHERE NAME LIKE ?";
		sql       += " ORDER BY ID ASC";
		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, "%"+name+"%");
		ResultSet rs            = pstmt.executeQuery();
		
		
		while(rs.next()){
			
			tag += "<tr>";
			tag += "<td>" + rs.getInt("id") + "</td>";
			tag += "<td>" + rs.getString("name") + "</td>";
			tag += "<td>" + rs.getString("userid") + "</td>";
			tag += "<td>" + rs.getString("passwd") + "</td>";
			tag += "<td>" + rs.getString("email") + "</td>";
			tag += "<td>" + rs.getString("regdate") + "</td>";
			tag += "<td><a href='deluser.jsp?id=" + rs.getInt("id") + "'>❌</a></td>";
			tag += "<tr>";			
		
		}
		tag += "</table>";
		
		rs.close();
		pstmt.close();
		conn.close();
	}
	
		
	// 출력할 html 만들기

%>    
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>Insert title here</title>
	<style>
		table, th, td {
			border : 1px solid black;
			border-collapse : collapse;
		}
		
		table {
			width : 80%; margin : 0 auto;
			
		}
		
		td {
			padding : 10px;	
		}
		
		a {
			text-decoration: none; 
		}
		
		h2 { text-align : center; }

		#t1, #t2 { width : 80%; margin : 0 auto; }

		#t1   td  { text-align : center; }
		#t1   td:nth-of-type(1) input {width:40px;}
		#t1   td:nth-of-type(2) input  {width:120px;}
		#t1   td:nth-of-type(3) input  {width:120px;}
		#t1   td:nth-of-type(4) input  {width:120px;}
		#t1   td:nth-of-type(5) input  {width:120px;}
		#t1   td:nth-of-type(6) input  {width:120px;}
	   
		#t1   [name="name"], [name="passwd"], [name="email"]    
      	{ border:2px solid red;   }
	</style>
	<script src="https://code.jquery.com/jquery-3.7.1.min.js" ></script>
	<script>
		$(function(){			
			$('#t2 tr').on({
				mouseover : function(){
					$(this).css({
						backgroundColor : "#333",
						color : "#ccc"						
					})
				},
				mouseout : function(){
					$(this).css({
						backgroundColor : "white",
						color : "black"					
					})
				},				
				dblclick : function (){
					console.dir(this);
					
					//js 식으로
					for(var i = 0; i < this.childNodes.length-1; i++){
						let td = this.childNodes[i];
						const input = document.querySelectorAll('#t1 input')[i];
						input.value = td.innerHTML;
					}
					/*
					// jquery 식으로 
					this.childNodes.forEach(function(td, i){
						$('#t1 input').eq(i).val(td.innerHTML);
					});*/
					
					//name = this.childNodes[1].innerHTML
		            //$('#t1  [name="name"]').val(name)
					alert(name);
				}
				
			});
		})
	</script>
</head>
<body>
	<h2>사용자 목록</h2>
	
	<form action = "updateuser.jsp" method = "GET">
		<table id = "t1">
			<tr>
				<td>번호</td>
				<td>이름</td>
				<td>아이디</td>
				<td>비밀번호</td>
				<td>이메일</td>
				<td>가입일</td>
			</tr>
			<tr>
				<td><input type = "text" name = "id" readonly value = "123"/></td>
				<td><input type = "text" name = "name" /></td>
				<td><input type = "text" name = "userid" readonly/></td>
				<td><input type = "text" name = "passwd" /></td>
				<td><input type = "text" name = "email" /></td>
				<td><input type = "text" name = "regdate" readonly/></td>
				<td><input type = "submit" value = "수정" /></td>
			</tr>
		</table>
	</form>
	
	<p><%= tag %></p>
	
	<a href = "tuserlist.html">HOME</a>
</body>
</html>